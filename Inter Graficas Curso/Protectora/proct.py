...
def menu():
    ...
    def dog():
        ...
        def nuevo_perro():
            ...
            chip_txt= Entry(dog, width=20, textvariable=id, background="azure")
            chip_txt.place(x=1000, y= 158)
            
            #Lugar
            lugar= Label(dog, text ="Lugar",background="azure",font=("Monaco",18))
            lugar.place(x=800, y=200)
            lugar_txt= Entry(dog, width=20, textvariable=id, background="azure")
            lugar_txt.place(x=1000, y= 198)
            #Raza
            raza= Label(dog, text ="Raza",background="azure",font=("Monaco",18))
            raza.place(x=800, y=240)
            raza_txt= Entry(dog, width=20, textvariable=id, background="azure")
            raza_txt.place(x=1000, y= 238)
            #agnos
            agnos= Label(dog, text ="Años",background="azure",font=("Monaco",18))
            agnos.place(x=800, y=280)
            agnos_txt= Entry(dog, width=20, textvariable=id, background="azure")
            agnos_txt.place(x=1000, y= 278)
            #Fecha
            fecha= Label(dog, text ="Fecha",background="azure",font=("Monaco",18))
            fecha.place(x=800, y=320)
            fecha_txt= Entry(dog, width=20, textvariable=id, background="azure")
            fecha_txt.place(x=1000, y= 318)
            #Vacunado
            vacunado= Label(dog, text ="Vacunado",background="azure",font=("Monaco",18))
            vacunado.place(x=800, y=360)
            vacunado_txt= Entry(dog, width=20, textvariable=id, background="azure")
            vacunado_txt.place(x=1000, y= 358)

            #Nombre
            nombre= Label(dog, text ="Nombre",background="azure",font=("Monaco",18))
            nombre.place(x=800, y=120)
            nombre_txt= Entry(dog, width=20, textvariable=id, background="azure")
            nombre_txt.place(x=1000, y= 118)

            #------------------------------------------------------ BOTONES DE PERRO ---------------------------------------------------
            """ IMPORTANTE !!!!!!!  Revisar este botón que al sacar la pantalla de perro sale directamente antes de que presionemos nuevo en el menú ... """
            agregar= Button(dog, text="Agregar", width= 8, background="blue", activebackground="blue", )
            agregar.place(x = 800, y= 400)


            

        Nuevo.add_command(label="Nueva alta", command= nuevo_perro)
        #-------- SALIR ----------------------------------------------------
        def off():
            dog.destroy()
            ventana2.destroy()
